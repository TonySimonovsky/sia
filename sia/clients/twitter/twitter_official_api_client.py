...
class SiaTwitterOfficial(SiaClient):
    ...
    def publish_post(
            ...
            media: dict = [],
            in_reply_to_tweet_id: str = None) -> str:
    ...
    def tweet_to_message(
            self,
            tweet: Tweet,
            author: TwpUser) -> SiaMessageGeneratedSchema:
    ...
    def search_tweets(
        ...
        since_id: str = None,
            client: tweepy.Client = None) -> TwpResponse:
        if not client:
            client = self.client
    ...
    def save_tweet_to_db(
            self,
            tweet: Tweet,
            author: TwpUser) -> SiaMessageSchema:
        ...
        try:
            ...
            if get_message_in_db:
                log_message(
                    self.logger, "info", self, f"Message with id {
            ...
            # if the tweet is not in the database
                ...
                #   to message object
                message_to_add = self.tweet_to_message(
            ...
            log_message(
                ...
                "error",
                self,
                f"Error saving tweet to database: {e}")

    def save_tweets_to_db(
            self,
            tweets: TwpResponse,
            exclude_own=True,
            exclude_responded_to=False) -> list[SiaMessageSchema]:
        messages = []
        ...
        for tweet in tweets.data:
            ...
            try:
                ...
                if flagged:
                    log_message(
                        self.logger, "info", self, f"The tweet (id {
                            tweet.id}) ({
                            tweet.text.replace(
                                '\n', ' ')}) was flagged: {
            ...
            if exclude_responded_to:
                message_responses_in_db = self.memory.get_messages(conversation_id=str(
                    tweet.id), author=self.character.twitter_username, flagged=2)
                if message_responses_in_db:
                    log_message(
                        self.logger, "info", self, f"Message with id {
                        ...
                        for included_tweet in tweets.includes['tweets']:
                            if included_tweet.id == ref_tweet_id:
                                try:
                                    ...
                                    log_message(
                                        self.logger, "error", self, f"Error adding referenced tweet: {e}")
    ...
    def printable_tweet(
        ...
        output_str += ' ' * indent_width + f"(likes: {
            public_metrics.get(
        ...
        for tweet in tweets.data:
            ...
            if 'referenced_tweets' in tweet:
                for ref_tweet in tweet.referenced_tweets:
                    ref_tweet_id = ref_tweet.id
                    ref_tweet_data = next(
                        (t for t in tweets.includes['tweets'] if t.id == ref_tweet_id), None)
                    if ref_tweet_data:
                        ref_author = next(
                            (user for user in tweets.includes['users'] if user.id == ref_tweet_data.author_id),
                            None)
    ...
    def decide_which_tweet_to_reply_to(
            self, tweets: list[SiaMessageSchema]) -> SiaMessageSchema:
        ...
        if self.testing:
            log_message(self.logger_testing, "info", self,
                        f"***Decision***:\n{decision}\n\n")
    ...
    def post(self):
        ...
        next_post_time = character_settings.character_settings.get(
            'twitter', {}).get('next_post_time', 0)
        next_post_datetime = datetime.fromtimestamp(next_post_time).strftime(
            '%Y-%m-%d %H:%M:%S') if next_post_time else "N/A"
        ...
        if self.character.platform_settings.get(
            "twitter",
            {}).get(
            "post",
            {}).get(
            "enabled",
                False) and time.time() > next_post_time:
                ...
                if tweet_id and tweet_id is not Forbidden:
                    ...
                    character_settings.character_settings = {
                        "twitter": {
                            ...
                            self.character.platform_settings.get(
                                "twitter",
                                {}).get(
                                "post_frequency",
                                2) *
    ...
    def reply(self):
        ...
        if self.character.responding.get("enabled", True):
            ...
            replies_messages = self.save_tweets_to_db(
                tweets=replies, exclude_own=True)
            ...
            responses_sent_this_hour = len(
                [r for r in responses_sent if r.wen_posted > datetime.now() - timedelta(hours=1)])
            ...
            if replies_messages:
                ...
                for r in replies_messages:
                    ...
                    log_message(self.logger, "info", self,
                                f"Processing reply: {r}")

                    # skipping flagged
                    if r.flagged:
                        log_message(self.logger, "info", self,
                                    f"Skipping flagged reply: {r}")
                        continue
                    ...
                    own_messages_count = sum(
                        1 for msg in current_conversation if msg.author == self.character.twitter_username)
                    if own_messages_count >= 3:
                        log_message(
                            self.logger, "info", self, f"Skipping conversation {
    ...
    def engage(self, testing_rounds=3, search_period_hours=24):
        ...
        if (not self.character.platform_settings.get("twitter", {}).get(
                "engage", {}).get("enabled", False)) and (not self.testing):
            return

        search_frequency = self.character.platform_settings.get(
            "twitter", {}).get(
            "engage", {}).get(
            "search_frequency", 1)
        ...
        if messages_to_engage_in_db:
            ...
            next_time_to_engage = latest_message.wen_posted + \
                timedelta(hours=search_frequency)
            is_time_to_engage = datetime.now() > next_time_to_engage
            if not is_time_to_engage and not self.testing:
                log_message(
                    self.logger, "info", self, f"Not the time to engage yet. Last time engaged: {
        ...
        for i in range(testing_rounds):
            ...
            start_time = (
                ...
                timedelta(
                    ...
                    i +
                    search_period_hours)).isoformat()
            end_time = datetime.now(
                timezone.utc) - timedelta(hours=search_frequency * i) - timedelta(seconds=23)

            # search for tweets to engage with
            tweets_to_engage = []
            for search_query in self.character.platform_settings.get(
                "twitter",
                {}).get(
                "engage",
                {}).get(
            ...
            if not tweets_to_engage:
                log_message(
                    ...
                    "info",
                    self,
                    f"No tweets found to engage with")
                continue
            if self.testing:
                log_message(
                    self.logger_testing, "info", self, f"***Tweets to engage with***:\n{
                        tweets_to_engage[0].printable_list(tweets_to_engage)}\n\n")

            # select a tweet to engage with
            tweet_to_respond = self.decide_which_tweet_to_reply_to(
                tweets_to_engage)
            if self.testing:
                log_message(self.logger_testing, "info", self,
            ...
            if not self.testing:
                ...
                log_message(
                    ...
                    "info",
                    self,
                    f"Published response with id: {tweet_id}")
    ...
    async def run(self):
        ...
        if not self.character.platform_settings.get(
                "twitter",
                {}).get(
                "enabled",
                True):
